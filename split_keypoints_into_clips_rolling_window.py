...
def process_all_keypoints(keypoints_dir, output_dir, annotation_file, frames_per_clip=100, stride=None, clip_annotations_file=None):
    ...
    Args:
        ...
        stride: Number of frames to advance for each new clip (rolling window stride)
               If None, non-overlapping clips will be created (stride = frames_per_clip)
        clip_annotations_file: Path to save clip annotations
    """
    
    # If stride is not specified, use frames_per_clip (non-overlapping clips)
    if stride is None:
    ...
    for keypoint_file in tqdm(keypoint_files, desc="Processing keypoint files"):
        try:
            ...
            # Calculate number of clips with rolling window
            num_clips = max(0, (num_frames - frames_per_clip) // stride + 1)
            
            print(f"Processing {video_name}: {num_frames} frames, {num_clips} clips (stride={stride})")
            
            # Process each clip using rolling window
            for clip_idx in range(num_clips):
                ...
                # Set clip name - use only the starting frame as the unique identifier
                anno['frame_dir'] = f"{patient_id}_{video_id}_frame{start_frame}"
    ...
    # Sort by patient ID, video ID, and then frame number
    sorted_clip_annotations = sorted(clip_annotations, key=lambda x: (
        # Extract patient_id, video_id, and frame number from the path
        ...
        int(os.path.basename(x[0]).split('_frame')[1].split('.')[0].replace("_aug", ""))  # frame number, removing file extension and _aug suffix
    ))
    ...
    print(f"Rolling window stride: {stride} frames")
